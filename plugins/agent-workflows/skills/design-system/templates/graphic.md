...
```tsx
<section id="concept-slug" className="min-h-screen snap-start flex flex-col justify-center px-8 lg:px-14 py-10">
...
- **section 容器(Screen)**:内容与 section 划分由调用技能决定;每节默认用 `min-h-screen snap-start flex flex-col justify-center`,主滚动容器 `snap-y snap-proximity`。内容超一屏时允许自然滚动,不用 snap-mandatory 硬卡,不为适配屏高拆散一个概念。
- **布局自由**:不许所有节都长一个样。按内容选布局——整屏陈述(动机/结论)、仪器 + 操控台分栏(demo + 控件 + 步骤)、网格(并列的「出现在哪里」卡片)、左右分栏(小结 ‖ 挑战)。**图形演示分栏时,图形/仪器在左,文字与控件在右。**「拆解」与它操控的 demo 放在同一 section,桌面同屏可见,小屏可自然堆叠滚动。
...
- **坐标纸舞台 `.stage`**(定义在工程 `index.css` 的 `@layer components`):`--color-canvas` 底 + 顶部 `--color-primary` 8% 径向辉光 + 28px 淡格坐标纸纹理(`--color-ink` 5% 的 1px 网格线);hover 时边框泛 primary、带同色外辉光(box-shadow 两层:1px 环 + 大面积柔光):
  ...
  ```
- **eyebrow 标签**:每个章节顶部一行 `font-mono text-xs uppercase tracking-[0.25em] text-muted`,前缀 20px 的 primary 短横线。标签概括本节概念或教学作用(如拆解/验证),不要求每个教学环节独立成节,不写装饰性标签。
